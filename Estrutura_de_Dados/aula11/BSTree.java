...
public class BSTree<T extends Comparable<? super T>> {   
   ...
   // Devolve o menor valor da árvore
    public T minValue() {
       BSTNode<T> cur = root;
   ...
   // Devolve o maior valor da árvore
    public T maxValue() {
       BSTNode<T> cur = root;
     ...
     // Conta quantos valores estão no intervalo [a,b]
   ...
   private int countBetween(BSTNode<T> n, T a, T b) {
      ...
      // valor < a → só interessa o lado direito
      if (n.getValue().compareTo(a) < 0) {
         return countBetween(n.getRight(), a, b);
      }

      // valor > b → só interessa o lado esquerdo
      if (n.getValue().compareTo(b) > 0) {
         return countBetween(n.getLeft(), a, b);
      }

      // caso intermédio: valor está entre [a,b]
